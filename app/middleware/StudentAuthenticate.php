...
class StudentAuthenticate
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        /** @var StudentAccessToken|null $tokenRecord */
        $tokenRecord = StudentAccessToken::where('token', $tokenHash)->find();
        if ($tokenRecord === null) {
            return api_error('访问令牌不存在或已失效', 401);
        }

        /** @var StudentAccessToken|null $currentTokenRecord */
        $currentTokenRecord = StudentAccessToken::where('student_id', (int) $tokenRecord->student_id)
            ->order('id desc')
            ->find();
        if ($currentTokenRecord === null || (int) $currentTokenRecord->id !== (int) $tokenRecord->id) {
            return api_error('当前账号已在其他设备登录', 401);
        }
        ...
        $student = Student::where('id', (int) $tokenRecord->student_id)->find();
        if ($student === null || (int) $student->status !== 1) {
            return api_error('学生账号不存在或已停用', 401);
        }
        ...
        if ($expiresAt !== null && ($expiresAt - $now) <= StudentAuthService::TOKEN_REFRESH_THRESHOLD_SECONDS) {
            $tokenRecord->expires_at = date('Y-m-d H:i:s', $now + StudentAuthService::TOKEN_TTL_SECONDS);
            $shouldSave = true;
        }
